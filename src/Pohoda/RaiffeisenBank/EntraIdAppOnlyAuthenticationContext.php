 ...
 * confirmed by hand against the daramis tenant. This class instead requests a
 * token from the Entra ID v2 endpoint with scope
 * "https://{tenant}.sharepoint.com/.default", which is what SharePoint Online
 * actually honors post-retirement.
 *
 ...
 * point to that app, not the retired ACS "SharePoint Add-In" principal.
 */
final class EntraIdAppOnlyAuthenticationContext implements IAuthenticationContext
{
     ...
     * one. Used by PohodaBankClientOffice::withSharePointRetry() to recover
     * from a transient Entra ID failure without rebuilding the whole context.
     */
