...
final class GraphSharePointClient
{
    ...
    /**
     * OFFICE365_PATH is configured tenant-wide with the default document
     * library's display name as its first segment (e.g. "Sdilene
     * dokumenty/DMA-5140016517_5500" - "Sdílené dokumenty" = "Shared
     * Documents", the default library). Graph's `drive/root:/` already
     * refers to that same default library, so that segment must be
     * stripped or Graph returns 404 itemNotFound - confirmed by hand
     * against the daramis tenant (both cases tested live: without the
     * first segment, HTTP 200 with the expected files, incl. a `webUrl`
     * that itself contains "Sdilene%20dokumenty/..."; with it, HTTP 404).
     */
